...
fn main() -> io::Result<()> {
    ...
    let output_path = out_dir.join("assets.rs");
    let mut file = File::create(&output_path)?;
    write!(
        &mut file,
        "pub const ASSETS: Dir = include_dir!(\"{}\");",
        assets.path().display()
    )?;

    // Copy Sciter DLL to output directory
    let sciter_dll_path = out_dir.join("sciter.dll");
    fs::copy("assets/sciter.dll", &sciter_dll_path)?;
